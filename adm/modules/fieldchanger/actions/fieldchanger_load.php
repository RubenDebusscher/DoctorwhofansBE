...
/**
 * @brief An HTTP REST action to load the options for a particular fieldchanger field.  Returns JSON
 * data.
 *
 * @section rest REST Interface
 *
 * @param string -table The name of the table where the fieldchanger field resides.
 * @param string -field The name of the fieldchanger field.
 * OR
 ...
 * @param string -fieldchanger-single-value If present, indicates that this should return only a single
 *                      value.
 * @param string -fieldchanger-id The ID of the single value to obtain.
 ...
 *                          If -fieldchanger-single-value is specified, then the single value
 ...
 * 				source table (i.e. the table containing the fieldchanger field).
 * 			</td>
 * 		</tr>
 * 		<tr>
 * 			<td>500</td>
 ...
 * @endcode
 */
class actions_fieldchanger_load {

    /**
     * @brief Returns  the default error message for when a server error has occurred.
     * @param string $fieldname The name of the fieldchanger field on which the error occurred.
    ...
    function handle($params) {
        ...
        if (@$query['--fieldchanger-table']) {
            $tableName = $query['--fieldchanger-table'];
        }
        $table = Dataface_Table::loadTable($tableName);
        $field = $table->getField($query['-field']);
        //$table = Dataface_Table::loadTable($field['widget']['table']);
        if ( @$query['-fieldchanger-single-value'] ){
    ...
    function loadSingle(Dataface_Table $table, &$field, $query) {
        try {
            if ( !@$query['-fieldchanger-id']){
                throw new Exception("Please specify the -fieldchanger-id parameter");
            }
            ...
            if (!@$perms['view']) {
                ...
                throw new Exception("Failed to get value for fieldchanger because you don't have view permission");
            }
     ...
     *
     * @param Dataface_Table $table The table where the fieldchanger field resides.
     * @param array &$field The field definition of the fieldchanger field.
