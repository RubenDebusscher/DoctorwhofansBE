<?php
/**
@mainpage Xataface fieldchanger Module

<img src="http://media.weblite.ca/files/photos/Screen%20shot%202011-06-09%20at%2011.11.34%20AM.png?max_width=640"/>

@section synopsis Synopsis

The Xataface fieldchanger module adds a "dependent select" widget for use in forms.  A dependent select is one whose options are dependent on the state of other widgets in the form.  It listens for changes to other selects and updates its own options accordingly.

A common example is a form with select lists for country, province, and city.  The options in the "province" select list will depend on which option is selected in the "country" select list.  Similarly the options in the "city" widget depend on the "province" selection.   The fieldchanger widget is ideal of this scenario as it will live update the options according to the selections in the dependency fields.
...
<a href="http://weblite.ca/svn/dataface/modules/fieldchanger/trunk">http://weblite.ca/svn/dataface/modules/fieldchanger/trunk</a>


@section installation Installation

	-# Copy the summary directory into your modules directory. i.e.: @code
	modules/fieldchanger
	@endcode
	-# Add the following to the [_modules] section of your app's conf.ini file: @code
	modules_fieldchanger=modules/fieldchanger/fieldchanger.php
	@endcode
...
To configure a field to use the fieldchanger widget, just set its fields.ini file widget:type directive to "fieldchanger".  You should also set the following directives:
...
The key to the fieldchanger widget lies in the widget:filters directive which specifies either a static or a dynamic filter that can be used to filter the options in the fieldchanger widget.  A static filter is just
any string that will be used to filter the particular field of the table specified by the widget:table directive.  With a static filter (or no filter), the fieldchanger will behave essentially the same
...
in the form whose value should be used to filter the options in this fieldchanger widget.  It is signified by a "$" prefix followed by the name of the field whose value we wish to filter on.
...
In cases where you have specified a dynamic filter on the fieldchanger, you may want to specify an alternate filter to use in the case where the field upon which the fieldchanger depends has not been selected or is left blank.  
...
- jQuery UI is used to provide a more user-friendly experience.  In particular it's internal dialog module is used for adding records to the fieldchangers.  (<a href="http://jqueryui.com/">http://jqueryui.com</a>)
