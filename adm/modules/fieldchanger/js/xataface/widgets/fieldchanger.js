...
//require-css <xataface/widgets/fieldchanger.css>  
(function () {
    var $ = jQuery;
    ...
    /**
     * Updates the values for a fieldchanger list.  This is usually called in 
     * response to a change in one of the selects that this fieldchanger
     * is dependent upon.
     *
     * @param {HTMLElement} select The fieldchanger <select> element that is to be updated.
    ...
    function updateValuesFor(select, filters, callback) {
        ...
        var selector = $(select).parent().find('select.xf-fieldchanger-selector').get(0);
        var tablename = $(select).attr("data-xf-table");
        var fieldname = $(select).attr('data-xf-field');

        var url = DATAFACE_SITE_HREF;
        var q = {
            '-action': 'fieldchanger_load',
            //'-table': tablename,
            '--fieldchanger-table': tablename,
            '-table': $(select).attr('data-xf-fieldchanger-options-table'),
            '-field': fieldname
        };
    ...
    function addOptionFor(select, filters) {
        var tableName = $(select).attr("data-xf-fieldchanger-options-table");
        if (!tableName)
            return;
        ...
        if ($(select).attr('data-xf-fieldchanger-dialogSize')) {
            var dialogSize = $(select).attr('data-xf-fieldchanger-dialogSize');
        ...
        if ($(select).attr('data-xf-fieldchanger-dialogMargin')) {
            var dialogMargin = $(select).attr('data-xf-fieldchanger-dialogMargin');
    ...
    function installFrozen(node){
        if ($(node).attr('data-fieldchanger-isdecorated')) {
            $(node).parent().find('.fieldchanger-fragment').remove();
        }
        var tablename = $(node).attr("data-xf-table");
        var fieldname = $(node).attr('data-xf-field');
        
        var fragment = $('<span>').addClass('fieldchanger-fragment');
        var url = DATAFACE_SITE_HREF;
        var q = {
            '-action': 'fieldchanger_load',
            //'-table': tablename,
            '--fieldchanger-table': tablename,
            '-table': $(node).attr('data-xf-fieldchanger-options-table'),
            '-field': fieldname,
            '-fieldchanger-single-value' : '1',
            '-fieldchanger-id' : $(node).val()
        };
    ...
    registerXatafaceDecorator(function (node) {
        $('input.xf-fieldchanger', node).each(function () {
            if ( $(this).attr('data-fieldchanger-frozen')){
                installFrozen(this);
                return;
            }
            if ($(this).attr('data-fieldchanger-isdecorated')) {
                $(this).parent().find('.fieldchanger-fragment').remove();
            }
            $(this).attr('data-fieldchanger-isdecorated', 1);
            var self = this;

            $(self).hide();
            var select = $('<select></select>')
                    .addClass('xf-fieldchanger-selector')
                    .addClass('fieldchanger-fragment')
                    .attr('data-xf-fieldchanger-dialogMargin', $(self).attr('data-xf-fieldchanger-dialogMargin'))
                    .attr('data-xf-fieldchanger-dialogSize', $(self).attr('data-xf-fieldchanger-dialogSize'))
                    .change(function () {
                        $(self).val($(this).val());
                        $(self).trigger('change');
                    })
            ...
            var filtersAttr = $(this).attr('data-xf-fieldchanger-filters');
            var filters = {};
            ...
            if ($(self).attr("data-xf-fieldchanger-perms-new")) {
                // We only add this button if the user has permission 
                // to add new records to the target table.
                $('<a class="fieldchanger-fragment"><img src="' + DATAFACE_URL + '/images/add_icon.gif"/></a>')
                        .addClass('xf-fieldchanger-add-btn')
                        .click(function () {
