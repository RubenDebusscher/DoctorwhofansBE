...
class Dataface_Record
{
     ...
     * @private
     */
    var $_relatedValuesLoaded = [];

    /**
     * @private
     */
    var $_numRelatedRecords = [];
    /**
     * @private
     */
    ...
    function getActions($params = [])
    {
        ...
        $parent = &$this->getParentRecord();
        if (isset($parent)) {
            $actions = array_merge_recursive_unique(
                $parent->getActions($params),
                $actions
            );
        }
        return $actions;
    ...
    {
        if (!isset($this->_relationshipRanges)) {
            $this->_relationshipRanges = [];
        }
        $this->_relationshipRanges[$relationshipName] = [$lower, $upper];
    }
    ...
    function _relatedRecordLoaded($relname, $index, $where = 0, $sort = 0)
    {
        ...
        return isset(
            $this->_relatedValuesLoaded[$relname][$where][$sort][$blockNumber]
        ) and
            $this->_relatedValuesLoaded[$relname][$where][$sort][$blockNumber];
    }
    ...
    function _translateRangeToBlocks($lower, $upper)
    {
        ...
        return [intval($lowerBlock), intval($upperBlock)];
    }
    ...
    {
        return isset(
            $this->_relatedValuesLoaded[$relname][$where][$sort][$block]
        ) and $this->_relatedValuesLoaded[$relname][$where][$sort][$block];
    }
     ...
     * @private
     */
    function _loadRelatedRecordBlock(
        $relname,
        $block,
        $where = 0,
        $sort = 0,
        ...
        if ($this->_relatedRecordBlockLoaded($relname, $block, $where, $sort)) {
            return true;
        }
        ...
        if (!is_object($relationship)) {
            throw new Exception(
                df_translate(
                    ...
                    ["relationship" => $relname, "retval" => $relationship]
                ),
                E_USER_ERROR
            );
        }
        ...
        $sql = $this->parseString(
            $relationship->getSQL($this->loadBlobs, $where, $sort)
        );
            ...
            throw new Exception(
                xf_db_error($this->_table->db) .
                    df_translate(
                        ...
                        "Error loading related records for relationship '$relname' in table '" .
                            $this->_table->tablename .
                            "'.  There was a problem performing the sql query '$sql'. The Mysql error returned was '" .
                            xf_db_error($this->_table->db),
                        [
                            "relationship" => $relname,
                            "table" => $this->_table->tablename,
                            "xf_db_error" => xf_db_error($this->_table->db),
                            "sql" => $sql,
                        ]
                    ),
                E_USER_ERROR
        ...
        foreach ($res as $row) {
            ...
            foreach ($row as $key => $value) {
                if (strpos($key, "__") === 0) {
                    ...
                    $record_row[$key] = $this->_table->parse(
                        $relname . "." . $key,
                        $value
                    );
                }
            ...
            $this->_relatedValues[$relname][$where][$sort][
                $index++
            ] = &$record_row;
            $this->_relatedMetaValues[$relname][$where][$sort][
                $index - 1
            ] = &$meta_row;
            if ($rawSort >= 0 and $rawSort !== $sort) {
                $this->_relatedValues[$relname][$where][$rawSort][
                    $index - 1
                ] = &$record_row;
                $this->_relatedMetaValues[$relname][$where][$rawSort][
                    $index - 1
                ] = &$meta_row;
            }
            unset($record_row);
    ...
    function numRelatedRecords($relname, $where = 0)
    {
        if (!isset($this->_numRelatedRecords[$relname][$where])) {
            ...
            //if ( $where !== 0 ){
            $sql = $this->parseString(
                $relationship->getSQL($this->loadBlobs, $where)
            );
            //} else {
            //	$sql = $this->parseString($relationship->_schema['sql']);
            //}
            ...
            if (!$res and !is_array($res)) {
                ...
                throw new Exception(
                    df_translate(
                        ...
                        "Error calculating the number of related records there are for the relationship '$relname' in the table '" .
                            $this->_table->tablename .
                            "'.  There was a problem performing the sql query '$sql'.  The MYSQL error returned was '" .
                            xf_db_error($this->_table->db) .
                            "'.\n<br>",
    ...
    function &getRelatedRecords(
        ...
        if (!is_bool($multipleRows) and intval($multipleRows) > 0) {
            ...
            is_string($multipleRows) and
            intval($multipleRows) === 0 and
            $multipleRows !== "0"
        ) {
        ...
        list($defaultStart, $defaultEnd) = $this->getRelationshipRange(
            $relname
        );
        if ($start === null) {
        ...
        $sortArr = $sort === 0 ? [] : array_map("trim", explode(",", $sort));
        ...
        for ($i = $range[0]; $i <= $range[1]; $i++) {
            $res = $this->_loadRelatedRecordBlock(
                $relname,
                $i,
                $where,
                $sort,
        ...
        if ($multipleRows === true) {
            ...
            for ($i = $start; $i < $start + $limit; $i++) {
                if (
                    !isset($this->_relatedValues[$relname][$where][$sort][$i])
                ) {
                    continue;
                }
            ...
            return $out;
        } elseif (is_array($multipleRows)) {
            throw new Exception(
                "Unsupported feature: using array query for multiple rows in getRelatedRecords",
                E_USER_ERROR
            );
            // we are searching using a query
            foreach (
                array_keys($this->_relatedValues[$relname][$where][$sort])
                as $rowIndex
            ) {
                $row =
                    &$this->_relatedValues[$relname][$where][$sort][$rowIndex];
                $match = true;
                foreach ($multipleRows as $key => $value) {
                    ...
                    if (
                        $nvalue !=
                        $this->_table->normalize($fullpath, $rowIndex)
                    ) {
    ...
    function getChildren($start = null, $limit = null)
    {
        ...
        if (isset($delegate) and method_exists($delegate, "getChildren")) {
            ...
            return $children;
        } elseif ($rel = &$this->_table->getChildrenRelationship() !== null) {
            $it = $this->getRelationshipIterator(
                $rel->getName(),
                $start,
                $limit
            );
    ...
    function &getParent()
    {
        ...
        if (isset($delegate) and method_exists($delegate, "getParent")) {
            ...
            return $parent;
        } elseif ($rel = &$this->_table->getParentRelationship() !== null) {
     ...
     * @since 0.5
     */
    function getRelationshipIterator(
        $relationshipName,
        $start = null,
        $limit = null,
        $where = 0,
        ...
        if (!$sort) {
            ...
            if (PEAR::isError($relationship)) {
                throw new Exception(
                    "Relationship $relationship could not be found: " .
                        $relationship->getMessage()
                );
            }
            $order_column = $relationship->getOrderColumn();
            if (!PEAR::isError($order_column) and $order_column) {
        ...
        return new Dataface_RelationshipIterator(
            $this,
            $relationshipName,
            $start,
            $limit,
     ...
     */
    function &getRelatedRecordObjects(
        $relationshipName,
        $start = null,
        $end = null,
        $where = 0,
    ...
    function moveUp($relationship, $index)
    {
        ...
        if (PEAR::isError($order_col)) {
            return $order_col;
        }
        $order_table = &$r->getTable($order_col);
        if (PEAR::isError($order_table)) {
            return $order_table;
        }

        if ($index == 0) {
            return PEAR::raiseError("Cannot move up index 0");
        }
        ...
        if (
            intval($prev_record->val($order_col)) ==
            intval($curr_record->val($order_col))
        ) {
    ...
    function sortRelationship($relationship, $start = null, $subset = null)
    {
        ...
        if (isset($start)) {
            ...
            $aExists = $start > 0;
            $countA = count($subset);
            $B = &$this->getRelatedRecordObjects(
                $relationship,
                max($start - 1, 0),
                $countA + 2
            );
            ...
            if (
                (!$aExists or
                    $B[0 + $AOffset]->val($order_col) >
                        $B[0]->val($order_col)) and
                (!$dExists or
                    $B[$countA - 1 + $AOffset]->val($order_col) <
                        $B[$countB - 1]->val($order_col)) and
                $B[$countA - 1 + $AOffset]->val($order_col) -
                    $B[0 + $AOffset]->val($order_col) >=
                    $countA - 1
            ) {
    ...
    function setValue($key, $value, $index = 0)
    {
        ...
        if (strpos($key, ".") !== false) {
            throw new Exception(
                "Unsupported operation: setting value on related record.",
                E_USER_ERROR
            );
        }
        ...
        // This is a local field
            ...
            if (!array_key_exists($key, $this->_table->fields())) {
                if (array_key_exists($key, $this->_table->transientFields())) {
                    ...
                    !array_key_exists($key, $this->_table->graftedFields(false))
                ) {
                    $parent = &$this->getParentRecord();
    ...
    function &getValue(
        ...
        if (
            isset($this->cache[__FUNCTION__][$fieldname][$index][$where][$sort])
        ) {
            return $this->cache[__FUNCTION__][$fieldname][$index][$where][
                $sort
            ];
        }
        ...
        if (strpos($fieldname, ".") === false) {
            ...
            if (!isset($this->_values[$fieldname])) {
                ...
                if (
                    $delegate !== null and
                    method_exists($delegate, "field__$fieldname")
                ) {
                    ...
                    if (array_key_exists($fieldname, $this->_transientValues)) {
                        ...
                        isset($transientFields[$fieldname]["relationship"]) and
                        $transientFields[$fieldname]["widget"]["type"] == "grid"
                    ) {
                        $out = [];
                        $rrecords = &$this->getRelatedRecordObjects(
                            $transientFields[$fieldname]["relationship"],
                            "all"
                        );
                        $currRelationship = &$this->_table->getRelationship(
                            $transientFields[$fieldname]["relationship"]
                        );
                        ...
                        foreach ($rrecords as $rrecord) {
                            ...
                            foreach (array_keys($row) as $row_field) {
                                $ptable = &$rrecord->_relationship->getTable(
                                    $row_field
                                );
                                $precord = &$rrecord->toRecord(
                                    $ptable->tablename
                                );
                                ...
                                $row["__permissions__"][
                                    $row_field
                                ] = $precord->getPermissions([
                                    "field" => $row_field,
                                ]);
                                if (isset($relKeys[$row_field])) {
                                    unset(
                                        $row["__permissions__"][$row_field][
                                            "edit"
                                        ]
                                    );
                                }
                        ...
                        isset($transientFields[$fieldname]["relationship"]) and
                        $transientFields[$fieldname]["widget"]["type"] ==
                            "checkbox"
                    ) {
                        $out = [];
                        $rrecords = &$this->getRelatedRecordObjects(
                        ...
                        if (
                            isset($delegate) and
                            method_exists($delegate, $fieldname . "__init")
                        ) {
                        ...
                        if (!isset($out)) {
                            ...
                            if (
                                isset($appdel) and
                                method_exists($appdel, $methodname)
                            ) {
                                $out = $appdel->$methodname(
                                    $this,
                                    $transientFields[$fieldname]
                                );
                                if (isset($out)) {
                                    $this->_transientValues[$fieldname] = $out;
                                }
                    ...
                    $parent = &$this->getParentRecord() and
            ...
            if (isset($out)) {
                ...
                $this->cache[strval(__FUNCTION__)][strval($fieldname)][$index][
                    $where
                ][$sort] = $out;
            }
            return $out;
            ...
            $this->cache[__FUNCTION__][$relationship . "." . $fieldname][
                $index
            ][$where][$sort] = &$rec[$fieldname];
    ...
    function &getValues($fields = null, $index = 0, $where = 0, $sort = 0)
    {
        ...
        $fields =
            $fields === null
                ? array_keys($this->_table->fields(false, true))
                : $fields;
    ...
    function getValueAsString($fieldname, $index = 0, $where = 0, $sort = 0)
    {
        ...
        if (
            $delegate !== null and
            method_exists($delegate, $rel_fieldname . "__toString")
        ) {
    ...
    function array2string($value)
    {
        ...
        if (is_array($value)) {
            ...
            return implode($delim, array_map([&$this, "array2string"], $value));
        }
        return "";
    }
    ...
    {
        $keys = is_array($fields)
            ? $fields
            : array_keys($this->_table->fields(false, true));
        $values = [];
        foreach ($keys as $key) {
            $values[$key] = $this->getValueAsString(
                $key,
                $index,
                $where,
                $sort
    ...
    function getSerializedValue($fieldname, $index = 0, $where = 0, $sort = 0)
    {
        ...
        return $s->serialize(
            $fieldname,
            $this->getValue($fieldname, 0, $where, $sort)
        );
    }
    ...
    function display(
        ...
        if (!is_string($fieldname)) {
            throw new Exception(
                "Expected fieldname to be string but found " . $fieldname
            );
        }
        if (
            !$thumbnail and
        ...
        if (strpos($fieldname, ".") === false) {
            ...
            if (
                $this->secureDisplay and
                !Dataface_PermissionsTool::view($this, ["field" => $fieldname])
            ) {
                $del = &$this->_table->getDelegate();
                if ($del and method_exists($del, "no_access_text")) {
                    return $del->no_access_text($this, ["field" => $fieldname]);
                } else {
                    return "NO ACCESS";
                }
            }
            ...
            $this->cache[__FUNCTION__][$fieldname][$index][$where][
                $sort
            ] = $out;
            return $out;
        }
        ...
        if (
            $delegate !== null and
            method_exists($delegate, $fieldname . "__display")
        ) {
        ...
        if (PEAR::isError($field)) {
            throw new Exception(
                "Attempt to get non-existent field $fieldname in table " .
                    $this->_table->tablename .
                    ": " .
                    $field->getMessage()
            );
        ...
        if (
            $this->_table->isBlob($fieldname) or
            $this->_table->isContainer($fieldname) and
                (@$field["secure"] or @$field["transform"])
        ) {
            $thumb = null;
            if ($this->getLength($fieldname) > 0) {
                ...
                foreach ($keys as $key) {
                    $qstr .=
                        "&$key" .
                        "=" .
                        $this->strval($key, $index, $where, $sort);
                }
                ...
                $thumb =
                    ...
                    @$query["-action"] and
                    @$field["transform"])
                        ? $this->getThumbnailTypeForAction(
                            $fieldname,
                            $query["-action"]
                        )
                        : null;
            ...
            return $out;
        } elseif ($this->_table->isContainer($fieldname)) {
            $field = &$this->_table->getField($fieldname);
            ...
            if (isset($valuelist) && !PEAR::isError($valuelist)) {
                ...
                //else if ( isset( $valuelist[$value]) ){
                    ...
                    if (isset($valuelist[$value])) {
                        ...
                        is_numeric($value) and isset($valuelist[intval($value)])
                    ) {
                ...
                if (isset($parent) and $parent->_table->hasField($fieldname)) {
                    return $parent->display(
                        $this->_table->getDisplayField($fieldname),
                        $index,
                        $where,
                        $sort
                    );
                }
                $out = $this->getValueAsString(
                    $this->_table->getDisplayField($fieldname),
                    $index,
                    $where,
                    $sort
    ...
    function htmlValue(
        ...
        if (
            isset($delegate) &&
            method_exists($delegate, $fieldname . "__htmlValue")
        ) {
            ...
            if (
                is_string($res) and
                DATAFACE_USAGE_MODE == "edit" and
                $this->checkPermission("edit", ["field" => $fieldname]) and
                !$this->_table->isMetaField($fieldname)
            ) {
                $res =
                    '<span id="' .
                    df_escape($domid) .
        ...
        Dataface_Application::getInstance()->fireEvent(
            "Dataface_Record__htmlValue",
            $event
        );
        if (isset($event->out)) {
            return $event->out;
        ...
        if (
            ...
            if ($del and method_exists($del, "no_access_link")) {
                $link = $del->no_access_link($this, ["field" => $fieldname]);
        ...
        if (
            $isImage or
            $this->_table->isBlob($fieldname) or
            $this->_table->isContainer($fieldname)
        ) {
            if ($this->getLength($fieldname, $index, $where, $sort) > 0) {
                if ($isImage) {
									$localFolder =$this->strval($field["localFolder"], $index, $where, $sort);
									if($localFolder!=""){
										$val=str_replace("///","/".$localFolder."/",$val);
                    ...
                    $file_icon = df_translate(
                        $this->getMimetype($fieldname, $index, $where, $sort) .
                            " file icon",
                        df_absolute_url(DATAFACE_URL) .
                            "/images/document_icon.gif"
                    );
                    $val =
                        '<img class="xf-container-field" src="' .
                        df_escape($file_icon) .
                        '"/><a href="' .
                        $val .
                        '" target="_blank"';
    ...
    function thumbnail(
        ...
        if (!$params) {
            $params = "max_width=75&max_height=100";
        }
        $thumb = null;
    ...
    function getThumbnailTypes($fieldname)
    {
        ...
        if (!$field) {
            throw new Exception(
                "Attempt to get thumbnail types for nonexistent field $fieldname"
            );
        }
        $event = new StdClass();
        ...
        $event->out = [];
        Dataface_Application::getInstance()->fireEvent(
            "Dataface_Record.getThumbnailTypes",
            $event
        );
        if ($event->consumed) {
            return $event->out;
    ...
    function removePropertyChangeListener($key, &$listener)
    {
        ...
        if (!isset($callback)) {
            unset($this->propertyChangeListeners[$key]);
        } elseif (isset($this->propertyChangeListeners[$key])) {
            if (
                ($index = array_search(
                    $listener,
                    $this->propertyChangeListeners[$key]
                )) !== false
            ) {
    ...
    function firePropertyChangeEvent($key, $oldValue, $newValue)
    {
        ...
        foreach ($keys as $key) {
            if (!isset($this->propertyChangeListeners[$key])) {
                continue;
            }
            foreach (
                array_keys($this->propertyChangeListeners[$key])
                as $lkey
            ) {
                $this->propertyChangeListeners[$key][$lkey]->propertyChanged(
                    $this,
                    $origKey,
                    $oldValue,
                    $newValue
    ...
    function propertyChanged(&$source, $key, $oldValue, $newValue)
    {
        ...
        if (
            is_a($source, "Dataface_Record") and
            is_a($parentRecord, "Dataface_Record") and
            $source->_id === $parentRecord->_id
        ) {
            ...
            if ($key_index === false) {
                throw new Exception(
                    "An error occurred trying to find the index of the parent's key.  This is a code error that should be fixded by the developer.",
                    E_USER_ERROR
                );
            }
            ...
            if (!isset($okey_names[$key_index])) {
                throw new Exception(
                    "Attempt to keep the current record in sync with its parent but they seem to have a different number of primary keys.  To use Dataface inheritance, tables must have a corresponding primary key.",
                    E_USER_ERROR
                );
            }
    ...
    function getJoinRecord($tablename, $nullIfNotFound = false)
    {
        ...
        if (!$record) {
            ...
            $record = new Dataface_Record($tablename, []);
     ...
     */
    function checkPermission($perm, $params = [])
    {
    ...
    function validate($fieldname, $value, &$params)
    {
        ...
        if (
            $field["widget"]["type"] == "file" and
            @$field["validators"]["required"] and
            is_array($value) and
            $this->getLength($fieldname) == 0 and
        ...
        if ($res) {
            ...
            if (
                $delegate !== null and
                method_exists($delegate, $fieldname . "__validate")
            ) {
                /*
                 *
    ...
    function &getParentRecord()
    {
        if (!isset($this->_parentRecord)) {
            ...
            if (isset($parent)) {
                $this->_parentRecord = new Dataface_Record(
                    $parent->tablename,
                    []
                );
                foreach (array_keys($parent->keys()) as $key) {
                    $this->_parentRecord->addPropertyChangeListener(
                        $key,
                        $this
                    );
                }
    ...
    function recordChanged($checkParent = false)
    {
        ...
        foreach (array_keys($fields) as $fieldname) {
            if ($this->valueChanged($fieldname)) {
                return true;
            }
        }
        return false;
    ...
    function clearFlags()
    {
        ...
        foreach (array_keys($this->_relatedDirtyFlags) as $rel_name) {
            foreach (
                array_keys($this->_relatedDirtyFlags[$rel_name])
                as $field_name
            ) {
    ...
    function setFlag($fieldname, $index = 0)
    {
        if (strpos($fieldname, ".") !== false) {
            ...
            if (!isset($this->_relatedDirtyFlags[$path[0]])) {
                $this->_relatedDirtyFlags[$path[0]] = [];
            }
            $this->_relatedDirtyFlags[$path[0]][$path[1]] = true;
        } else {
            // This is a local field
    ...
    function isLoaded($fieldname)
    {
        ...
        return isset($this->_isLoaded[$fieldname]) and
            $this->_isLoaded[$fieldname];
    }
    ...
    function getLink($fieldname)
    {
        ...
        if (
            isset($delegate) and method_exists($delegate, $fieldname . "__link")
        ) {
            ...
            if (!isset($title)) {
                ...
                foreach (array_keys($fields) as $field_name) {
                    ...
                    if (isset($fields[$field_name]["title"])) {
                        ...
                        !isset($title) and $this->_table->isChar($field_name)
                    ) {
                ...
                if (!isset($title)) {
                    ...
                    $title = sprintf(
                        df_translate("Untitled table record", $titleStr),
    ...
    function getDescription()
    {
        if (($res = $this->callDelegateFunction("getDescription")) !== null) {
            return $res;
        } elseif ($descriptionField = $this->_table->getDescriptionField()) {
    ...
    function getByLine()
    {
        if (($res = $this->callDelegateFunction("getByline")) !== null) {
            return $res;
        } elseif ($bylineField = $this->_table->getBylineField()) {
    ...
    function getCreated()
    {
        if ($res = $this->callDelegateFunction("getCreated")) {
            return $res;
        } elseif ($createdField = $this->_table->getCreatedField()) {
            if (
                strcasecmp(
                    $this->_table->getType($createdField),
                    "timestamp"
                ) === 0
            ) {
    ...
    function getCreator()
    {
        if (($res = $this->callDelegateFunction("getCreator", -1)) !== -1) {
            return $res;
        } elseif ($creatorField = $this->_table->getCreatorField()) {
    ...
    function getLastModified()
    {
        if ($res = $this->callDelegateFunction("getLastModified")) {
            return $res;
        } elseif ($lastModifiedField = $this->_table->getLastUpdatedField()) {
            if (
                strcasecmp(
                    $this->_table->getType($lastModifiedField),
                    "timestamp"
                ) === 0
            ) {
        ...
        if (!isset($this->pouch["__mtime"])) {
            $sql =
                "select mtime from dataface__record_mtimes where recordhash='" .
                addslashes(md5($this->getId())) .
                "'";
            try {
                try {
                    $res = df_q($sql);
    ...
    function getBody()
    {
        if ($res = $this->callDelegateFunction("getBody")) {
            return $res;
        } elseif ($bodyField = $this->_table->getBodyField()) {
    ...
    function getBreadCrumbs()
    {
        ...
        if (($parent = $this->getParent()) !== null) {
            ...
            $bc[$this->getTitle()] = $this->getURL(["-action" => "browse"]);
            return $bc;
        }

        return [
            $this->_table->getLabel() => Dataface_LinkTool::buildLink([
                "-action" => "list",
                "-table" => $this->_table->tablename,
            ]),
            $this->getTitle() => $this->getURL(["-action" => "browse"]),
        ];
    }
    ...
    function getURL($params = [])
    {
        if (is_string($params)) {
            if (
                $this->_table->hasField($params) and
                ($this->_table->isBlob($params) or
                    $this->_table->isContainer($params))
            ) {
                if (
                    $this->secureDisplay and
                    !$this->checkPermission("view", ["field" => $params])
                ) {
    ...
    function getLength($fieldname, $index = 0, $where = 0, $sort = 0)
    {
        if (strpos($fieldname, ".") !== false) {
            ...
            $relatedRecord = new Dataface_RelatedRecord(
                $this,
                $relname,
                $record
            );
            $relatedRecord->setValues(
                $this->_relatedMetaValues[$relname][0][0][$index]
            );
    ...
    function save($lang = null, $secure = false)
    {
        ...
        return df_save_record(
            $this,
            $this->strvals(array_keys($this->_table->keys())),
            $lang,
            $secure
        );
    }
    ...
    function toJS($fields = null, $override = [])
    {
        $strvals = $this->strvals($fields);
        $out = [];
        foreach ($strvals as $key => $val) {
            if ($this->checkPermission("view", ["field" => $key])) {
                $out[$key] = $val;
            }
        }
...
class Dataface_RecordIterator
{
    ...
    function &next()
    {
        $out = new Dataface_Record(
            $this->_tablename,
            $this->_records[current($this->_keys)]
        );
        next($this->_keys);
        return $out;
    }
...
class Dataface_RelationshipIterator
{
    ...
    var $_where;
    var $_sort;
    function Dataface_RelationshipIterator(
        &$record,
        $relationshipName,
        $start = null,
        $limit = null,
        ...
        $this->_sort = $sort;
        if ($start !== "all") {
            $this->_records = &$record->getRelatedRecords(
                $relationshipName,
                true,
                $start,
                $limit,
    ...
    function __construct(
        ...
        $this->Dataface_RelationshipIterator(
            $record,
            $relationshipName,
            $start,
            $limit,
    ...
    function &current()
    {
        $rec = new Dataface_RelatedRecord(
            $this->_record,
            $this->_relationshipName,
            $this->_records[current($this->_keys)]
        );
        $rec->setValues(
            $this->_record->_relatedMetaValues[$this->_relationshipName][
                $this->_where
            ][$this->_sort][current($this->_keys)]
        );
        return $rec;
    }
