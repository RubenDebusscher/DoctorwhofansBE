<?php
	...
	$stmt1 = $conn->prepare("SELECT page_Name, page_Link, COUNT(content__items.item_Id)as aantal, GROUP_CONCAT( DISTINCT management__languages.language_Name ) as Languages FROM management__pages LEFT JOIN content__items ON management__pages.page_Id = content__items.item_Page LEFT JOIN content__items_languages ON content__items_languages.IL_item_Id = content__items.item_Id LEFT JOIN management__languages ON management__languages.language_Id = content__items_languages.IL_language_Id GROUP BY page_Id;");
