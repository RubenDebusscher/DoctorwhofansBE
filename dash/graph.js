...
function buildpage(){
    ...
    $.ajax({type: "GET",url: "https://www.doctorwhofans.be/php/dash.php",dataType: 'json',cache: false})
    .done(
        function (resultaat) {
            populateChart('Chart1', 'pie', "Aantal pagina's per aantal elementen", resultaat['AantalPagesPerAantalElements']);
    ...
    //
    //createTable('Eighth', "Populaire tijden (oude versie)","Populartimes_old.php",'True');
    //createTable('Fourth', "Elementen per pagina","elementsperPage.php",'True');
    //createTable('Seventh','Talen per pagina','PHP/langarrayperpage.php','True');
...
function populateChart(el, type, tit, arr) {
    ...
    for (i = 0; i < arr.length; i += 1) {
        lab.push(arr[i].Aantal_elem);
        customtoolTips.push(arr[i].pages);
        dat.push(arr[i].Aantal_Pag);
...
function createTable(el,tit,arr,pag) {
    $('#'+el).prev().html(tit);
    $('#'+el).html("");
    if(arr =="No rows"){
        ...
        generateDynamicTable(arr,el,pag);
		...
		if(noOfContacts>0){
			...
			for (var i = 0; i < noOfContacts; i++) {
                ...
                if(el=="Fourth"){
                    ...
                    classes.push(myContacts[i][col[1]]);
                    bRow.className=myContacts[i][col[1]];
                }else if(el=="Fourth" || el=="Sixth"){
                    classes.push(myContacts[i][col[1]]);
                    bRow.className=myContacts[i][col[1]];
...
function GetListLinks(arr,existingpages) {
    ...
    $('#items').empty();
            ...
            for (i = 0; i < arr.length; i += 1) {
                $('#Links').append("<li>" + arr[i].A_Waarde + "</li>");
            }
            ...
            for (var i = 0; i < links.length; i++) {
                if (links[i].href.includes("doctorwhofans")===true && links[i].pathname.includes("?") == false && links[i].pathname.includes("@") ==false && links[i].pathname.includes("/API")==false && links[i].pathname.includes("#")==false && links[i].pathname.includes(".php")==false) {
                    ...
                    NAME_ARRAY.push(links[i].pathname);
                }
            ...
            $('#Links').empty();
            getBrokenLinks(existingpages)
       
}


function resolveAfter2Seconds(existingpages) {
    ...
    setTimeout(function() {
        ...
        for(var i= 0;i<uniquelinks.length;i++){
            ...
            if(index < 0){
                if (typeof(existingpages.find(elem=>elem.Page_Link == uniquelinks[i].replace('.html','')))==undefined){

                    $("#items").append("<li class="+NAME_ARRAY[i].split('/').join('__').replace('.html','')+">"+NAME_ARRAY[i]+"</li>");
                }
            }
        };
        if($('#items').is(':empty')){
