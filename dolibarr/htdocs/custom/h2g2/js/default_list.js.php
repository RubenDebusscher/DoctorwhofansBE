	...
	if (container) {
		...
		columns.forEach(column => {
			...
			if (inputs.length > 0) {
				...
				if (select) {
					if (!select.classList.contains('select-include-zero') && select.value > 0) { // This select doesn't includes 0 as a value
						filters[select.name] = {'value': select.value, 'operator': '='};
					} else if (select.classList.contains('select-include-zero') && select.value >= 0) {  // This select includes 0 as a value
						filters[select.name] = {'value': select.value, 'operator': '='};
					} else {
