...
function buildMultiEntriesButton($mainBtn, $entries, $direction = true)
{
	...
	if ($mainBtn['disabled'] ?? false) {
		...
		if ($mainBtn['colorbtn'] ?? false) {
			$ret .= '<a title="'. $mainBtn['title'] .'" href="' . $mainBtn['href'] . '" style="background:' . $mainBtn['colorbtn'] . '" class="butAction h2g2multiselect__button classfortooltip' . $morecss . '">';
		} else {
	...
	if (!$withoutAction) {
		...
		$ret .= '<a href="#" style="background:' . ($mainBtn['colorbtn'] ?? "") . '"class="butAction h2g2multiselect__chevron ' . $str_directioninv . '"><i class="fas fa-chevron-' . $str_directioninv . '"></i> </a>';
	}
	...
	foreach ($entries as $entry) {
		...
		$ret .= '<div class="h2g2multiselect__options-entry ' . (!empty($entry['disabled']) ? 'disabled' : '') . ' h2g2tooltip__initiator">';
		$ret .= '<div class="h2g2multiselect__options-entry_icon">';
		if ($entry['pictocolor'] ?? false) {
			$entry['picto'] = str_replace('class=', 'style="color:' . $entry['pictocolor'] . '" class=', $entry['picto']);
		}
		$ret .= $entry['picto'];
		$ret .= '</div>';
		$ret .= '<div class="h2g2multiselect__options-entry_label">';
		if (!empty($entry['disabled'])) {
			$ret .= '<a href="#">' . $entry['label'] . '</a>';
		} else {
			$ret .= '<a href="' . $entry['href'] . '">' . $entry['label'] . '</a>';
		}
		$ret .= '</div>';
		$ret .= '</div>';
		$ret .= '</div>';
