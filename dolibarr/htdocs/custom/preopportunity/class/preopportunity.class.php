...
class PreOpportunity extends CommonObject
{
	...
	public $fields=array(
		...
		"source" => array("type"=>"sellist:c_preopportunity_source:label:rowid::(active:=:1)", "label"=>"Source", "enabled"=>"1", 'position'=>135, 'notnull'=>0, "visible"=>"1", "searchall"=>"1", "css"=>"maxwidth300 widthcentpercentminusxx",),
	...
	public function thirdPartyCreate(User $user) {
		...
		if(getDolGlobalString('DUPLICATE_CHECK_ON_THIRDPARTIES_NAME') || getDolGlobalString('DUPLICATE_CHECK_ON_THIRDPARTIES_EMAIL')) {
			// Check for the duplicates
			$sql = "SELECT t.rowid, t.email, t.nom FROM ".$this->db->prefix()."societe as t WHERE 1 =1";

			if(getDolGlobalString('DUPLICATE_CHECK_ON_THIRDPARTIES_NAME')) {
				if($this->companyname) {
					$namecheck = $this->companyname;
				} else {
					$namecheck = $this->firstname.' '.$this->lastname;
				}
				$sql .= " AND t.nom = '".$namecheck."'";
			}

			if(getDolGlobalString('DUPLICATE_CHECK_ON_THIRDPARTIES_EMAIL')) {
				$sql .= " AND t.email = '".$this->email."'";
			}
		...
		$object->state_id = $this->fk_departement;
		$object->country_id = $this->fk_pays;
	...
	public function contactCreation(User $user) {
		...
		if(getDolGlobalString('DUPLICATE_CHECK_ON_THIRDPARTIES_NAME') || getDolGlobalString('DUPLICATE_CHECK_ON_THIRDPARTIES_EMAIL')) {
			// Check for the duplicates in contacts table
			$sql = "SELECT t.rowid, t.email, t.lastname, t.firstname, t.fk_soc FROM ".$this->db->prefix()."socpeople as t WHERE 1 = 1 AND t.fk_soc = ".$this->fk_soc;
			if(getDolGlobalString('DUPLICATE_CHECK_ON_THIRDPARTIES_NAME')) {
				$sql .= " AND t.firstname = '".$this->firstname."' AND t.lastname = '".$this->lastname."'";
			}
	 ...
	 */
	public function fetchSource($id, $code = '')
	{
		$sql = "SELECT";
		$sql .= " t.rowid,";
		$sql .= " t.code,";
		$sql .= " t.label,";
		$sql .= " t.active";
		$sql .= " FROM ".$this->db->prefix()."c_preopportunity_source as t";
		if ($id) {
			$sql .= " WHERE t.rowid = ".((int) $id);
		} elseif ($code) {
			$sql .= " WHERE t.label = '".$this->db->escape($code)."'";
		}
		// echo $sql;exit;
		dol_syslog(get_class($this)."::fetch", LOG_DEBUG);
		$resql = $this->db->query($sql);
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				...
				$this->id = $obj->rowid;
				$this->sourcecode = $obj->code;
				$this->sourcelabel = $obj->label;
				$this->sourceactive = $obj->active;
			}
			$this->db->free($resql);
