 ...
 */
class today_timesheet_box extends ModeleBoxes
{
	/**
	 * @var string Alphanumeric ID. Populated by the constructor.
	 */
	public $boxcode = "lasttimesheets";

	/**
	 * @var string Box icon (in configuration page)
	 * Automatically calls the icon named with the corresponding "object_" prefix
	 */
	public $boximg = "timesheet@staff";
	...
	public function __construct(DoliDB $db, $param = '')
	{
		...
		$this->boxlabel = $langs->transnoentitiesnoconv("BoxTodayTimesheets");
	...
	public function loadBox($max = 5)
	{
		...
		//include_once DOL_DOCUMENT_ROOT . "/mymodule/class/mymodule.class.php";
                ...
                $timesheetstatic = new Timesheet($db);
                $userstatic = new User($db);

		// Populate the head at runtime
		$text = $langs->trans("BoxTodayTimesheets");
		$this->info_box_head = array(
			// Title text
			'text' => $text,
			// Add a link
			'sublink' => dol_buildpath('/staff/timesheet/index.php?mainmenu=hrm&leftmenu=plannedshift&action=show_day', 1),
			// Sublink icon placed after the text
			'subpicto' => 'list',
			// Sublink icon HTML alt text
			'subtext' => $langs->trans('BoxTodayTimesheets'),
			...
			'graph' => false
                ...
                if ($user->rights->staff->timesheet->read)
                {
                    $now = dol_now();
                    ...
                    $sql.= " AND status BETWEEN ".Timesheet::STATUS_PENDING." AND ".Timesheet::STATUS_PAID;
                    $sql.= " AND t.day = date('".$db->idate($now)."')";
                    ...
                    if ($result) {
                        ...
                        while ($line < $num) {
                            ...
                            $timesheetstatic->fk_user = $obj->fk_user;
                            $timesheetstatic->created_by = $obj->created_by;
                            $timesheetstatic->start_time = $obj->start_time;
                            $timesheetstatic->end_time = $obj->end_time;
                            $timesheetstatic->time_diff = $obj->time_diff;

                            $this->info_box_contents[$line][] = array(
                                'td' => 'align="left"',
                                'text' => $timesheetstatic->getNomUrl('',1),
                                'asis' => 1,
                            );
                            ...
                            $this->info_box_contents[$line][] = array(
                                'td' => 'align="left"',
                                'text' => $timesheetstatic->getTotalHours(),
                            );
                            ...
                            $this->info_box_contents[$line][] = array(
                                'td' => 'align="right"',
                                'text' => $timesheetstatic->LibStatut($obj->status,3),
                            );

                            $line++;
                        }

                        if ($num==0) $this->info_box_contents[$line][0] = array('td' => 'align="center"','text'=>$langs->trans("NoRecordedTimesheetsForToday"));
