 ...
 */
class pdf_rouget_numbering extends ModelePdfExpedition
{
	/**
	...
	public function __construct($db = 0)
	{
		...
		$this->db = $db;
		$this->name = "rouget with numbering";
			...
			if (file_exists($dir)) {
				...
				if (!empty($conf->incoterm->enabled)) {
					...
					if ($desc_incoterms) {
						...
						$pdf->SetFont('', '', $default_font_size - 1);
						$pdf->writeHTMLCell(190, 3, $this->posxpos - 1, $tab_top - 1, dol_htmlentitiesbr($desc_incoterms), 0, 1);
						$nexY = $pdf->GetY();
				...
				if (!empty($object->note_public) || !empty($object->tracking_number)) {
					...
					if (!empty($object->tracking_number)) {
						$pdf->writeHTMLCell(60, 4, $this->posxpos - 1, $tab_top - 1, $outputlangs->transnoentities("TrackingNumber")." : ".$object->tracking_number, 0, 1, false, true, 'L');
						...
						if (!empty($object->tracking_url)) {
							if ($object->shipping_method_id > 0) {
								...
								$pdf->SetFont('', 'B', $default_font_size - 2);
								$pdf->writeHTMLCell(60, 4, $this->posxpos - 1, $tab_top_alt, $label, 0, 1, false, true, 'L');
					...
					if (!empty($object->note_public)) {
						...
						$pdf->writeHTMLCell(190, 3, $this->posxpos - 1, $tab_top_alt, dol_htmlentitiesbr($object->note_public), 0, 1);
					}
				...
				for ($i = 0; $i < $nblines; $i++) {
                    ...
                    $pdf->MultiCell($this->posxdesc-$this->posxpos-0.8, 4, $i+1, 0, 'C', 0);
                    //##########################################################
