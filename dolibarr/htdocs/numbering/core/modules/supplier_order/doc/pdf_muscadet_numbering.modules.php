 ...
 */
class pdf_muscadet_numbering extends ModelePDFSuppliersOrders
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "muscadet with numbering";
		...
		if ($conf->fournisseur->commande->dir_output) {
			...
			if (file_exists($dir)) {
				...
				if (!empty($object->note_public)) {
					...
					$pdf->SetFont('', '', $default_font_size - 1);
					$pdf->writeHTMLCell(190, 3, $this->posxpos - 1, $tab_top - 1, dol_htmlentitiesbr($object->note_public), 0, 1);
					$nexY = $pdf->GetY();
