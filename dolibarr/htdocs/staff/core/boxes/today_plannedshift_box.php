 ...
 */
class today_plannedshift_box extends ModeleBoxes
{
	/**
	...
	public function __construct(DoliDB $db, $param = '')
	{
		...
		$this->boxlabel = $langs->transnoentitiesnoconv("BoxTodayPlannedShifts");
	...
	public function loadBox($max = 5)
	{
		...
		$text = $langs->trans("BoxTodayPlannedShifts");
		$this->info_box_head = array(
			// Title text
			'text' => $text,
			// Add a link
			'sublink' => dol_buildpath('/staff/timesheet/index.php?mainmenu=hrm&leftmenu=plannedshift&action=show_day&type=planned_shift', 1),
			// Sublink icon placed after the text
			'subpicto' => 'list',
			// Sublink icon HTML alt text
			'subtext' => $langs->trans('BoxTodayPlannedShifts'),
			...
			'graph' => false
                ...
                if ($user->rights->staff->plannedshift->read)
                {
                    ...
                    $sql.= " AND status BETWEEN ". PlannedShift::STATUS_WAITING_TO_CONFIRM." AND ".PlannedShift::STATUS_WAITING_TO_SUBMIT;
                    ...
                    if ($result) {
                        ...
                        while ($line < $num) {
                            ...
                            $this->info_box_contents[$line][] = array(
                                'td' => 'align="left"',
                                'text' => $plannedshiftstatic->getTotalHours(),
                            );
                            ...
                            $this->info_box_contents[$line][] = array(
                                'td' => 'align="right"',
                                'text' => $plannedshiftstatic->LibStatut($obj->status,3),
                            );

                            $line++;
                        }

                        if ($num==0) $this->info_box_contents[$line][0] = array('td' => 'align="center"','text'=>$langs->trans("NoRecordedPlannedShiftsForToday"));
