...
/**
 * \file    takeposcashiers/admin/setup.php
 * \ingroup takeposcashiers
 * \brief   TakeposCashiers setup page.
 */
...
require_once '../lib/takeposcashiers.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "takeposcashiers@takeposcashiers"));
...
$arrayofparameters = array(
	'TAKEPOSCASHIERS_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'TAKEPOSCASHIERS_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/takeposcashiers/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'TAKEPOSCASHIERS_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'TAKEPOSCASHIERS_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$page_name = "TakeposCashiersSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_takeposcashiers@takeposcashiers');

// Configuration header
$head = takeposcashiersAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "takeposcashiers@takeposcashiers");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("TakeposCashiersSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'TAKEPOSCASHIERS_'.strtoupper($myTmpObjectKey).'_ADDON';
